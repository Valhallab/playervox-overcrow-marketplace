...
const copy = {
  en: {
    ...
    legacy: "Legacy version", legacyPreview: "WIDGET",
    legacyHelp: "This version belongs to an earlier OverCrow release. Its listing remains available while an updated widget is prepared; it cannot be installed in the current app.",
    provider: "Publishes public data for other widgets",
  ...
  fr: {
    ...
    legacy: "Ancienne version", legacyPreview: "WIDGET",
    legacyHelp: "Cette version appartient à une ancienne version d’OverCrow. Sa fiche reste consultable en attendant un widget mis à jour ; elle ne peut pas être installée dans l’application actuelle.",
    provider: "Fournit des données publiques à d’autres widgets",
...
function target(value) {
  ...
  if (manifest.kind !== undefined) {
    // Display compatibility only: legacy packages never gain a native install link.
    const granted = manifest.capabilities;
    const available = manifest.availableLocales;
    if (policy.mode !== "production" || !["widget", "bundle", "provider"].includes(manifest.kind)
        || manifest.entrypoints !== undefined || manifest.permissions !== undefined
        || value.minHostApi !== 1 || value.maxHostApi !== 1
        || value.packageSize > 16 * 1024 * 1024
        || !Array.isArray(available) || available.length === 0 || available.length > 32
        || !available.every(locale) || new Set(available).size !== available.length
        || !available.includes("en") || !available.includes(manifest.defaultLocale)
        || !listing({ ...listed, defaultLocale: manifest.defaultLocale })
        || listed.localizations.length !== available.length
        || !listed.localizations.every((entry) => available.includes(entry.locale))
        || !exactKeys(granted, ["http", "gameData", "storage", "clipboardWrite", "provider"])) return false;
    if (!Array.isArray(granted.http) || granted.http.length > 16
        || new Set(granted.http).size !== granted.http.length
        || !granted.http.every((host) => string(host, 253) && host.includes(".")
          && host.split(".").every((label) => /^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$/u.test(label)))
        || !Array.isArray(granted.gameData) || granted.gameData.length > 1
        || !granted.gameData.every((feed) => feed === "overcrow.session.v1")
        || typeof granted.storage !== "boolean" || typeof granted.clipboardWrite !== "boolean"
        || typeof granted.provider !== "boolean") return false;
    if (!Array.isArray(manifest.games) || manifest.games.length > 32
        || !manifest.games.every((game) => game && game.platform === "steam"
          && typeof game.id === "string" && Number.isInteger(Number(game.id))
          && Number(game.id) > 0 && Number(game.id) <= 4294967295
          && String(Number(game.id)) === game.id)
        || new Set(manifest.games.map((game) => game.id)).size !== manifest.games.length
        || !Array.isArray(manifest.dependencies) || manifest.dependencies.length > 32
        || !manifest.dependencies.every((dependency) => dependency && extensionId(dependency.id)
          && version(dependency.version) && digest(dependency.sha256))
        || new Set(manifest.dependencies.map((dependency) => dependency.id)).size !== manifest.dependencies.length
        || (manifest.kind === "provider" && value.preview != null)) return false;
  } else {
    if (manifest.capabilities !== undefined || manifest.dependencies !== undefined
        || manifest.games !== undefined || manifest.minHostApi !== undefined
...
function validate(text) {
  ...
  const byIdentity = new Map(payload.targets.map((item) => [`${item.manifest.id}/${item.manifest.version}`, item]));
  const active = new Set();
  const complete = new Set();
  function visit(item) {
    if (active.has(item)) throw new Error("dependency");
    if (complete.has(item)) return;
    active.add(item);
    for (const dependency of item.manifest.dependencies || []) {
      const provider = byIdentity.get(`${dependency.id}/${dependency.version}`);
      if (!provider || provider.manifest.kind !== "provider" || provider.status !== "verified"
  ...
  for (const item of payload.targets) visit(item);
  // Freeze the complete legacy permission summary before hiding provider-only entries.
  for (const item of byId.values()) {
    if (item.manifest.kind === undefined) continue;
    const included = new Set([item]);
    for (const entry of included) {
      for (const dependency of entry.manifest.dependencies) {
        included.add(byIdentity.get(`${dependency.id}/${dependency.version}`));
      }
    }
    item.legacyCapabilities = [...included].map((entry) => entry.manifest.capabilities);
  }
  return [...byId.values()].filter((item) => item.manifest.kind !== "provider");
}
...
function details(item) {
  ...
  const values = [];
  if (item.manifest.kind !== undefined) {
    const capabilities = item.legacyCapabilities;
    const hosts = [...new Set(capabilities.flatMap((value) => value.http))];
    if (hosts.length) values.push(`${languageCopy.http}: ${hosts.join(", ")}`);
    if (capabilities.some((value) => value.gameData.length)) values.push(languageCopy.events);
    if (capabilities.some((value) => value.storage)) values.push(languageCopy.storage);
    if (capabilities.some((value) => value.clipboardWrite)) values.push(languageCopy.clipboard);
    if (capabilities.some((value) => value.provider)) values.push(languageCopy.provider);
    return values;
  }
...
function widgetPreview(item) {
  ...
  if (item.preview) {
    ...
    media.append(textElement("span", localized(item).name.slice(0, 2).toLocaleUpperCase(state.locale), "widget-monogram"),
      textElement("span", item.manifest.kind === undefined ? copy[state.locale].previewFallback : copy[state.locale].legacyPreview, "preview-caption"));
  }
  return media;
}
...
function card(item) {
  ...
  body.append(status);
  if (item.manifest.kind !== undefined) body.append(textElement("p", words.legacy, "badge"));
  body.append(footer);
...
function widgetDetails(item) {
  ...
  install.append(textElement("h2", item.manifest.kind === undefined ? words.installTitle : words.legacy),
  ...
  if (item.manifest.kind === undefined) {
    ...
    install.append(open, textElement("p", words.openHelp, "open-help"),
      textElement("p", words.flowActivate, "activation-note"));
  } else install.append(textElement("p", words.legacyHelp, "open-help"));
...
function renderCatalog() {
  ...
  const visible = state.targets.filter((item) => {
    const available = item.status === "verified" && item.manifest.kind === undefined;
    const matchesStatus = state.availability === "all" || (state.availability === "available" ? available : !available);
